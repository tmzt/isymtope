...
impl ObjectWriter<Expression<OutputExpression>, HtmlOutput> for DefaultHtmlWriter {
    fn write_object(&mut self, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, obj: &Expression<OutputExpression>) -> DocumentProcessingResult<()> {
        debug!("ObjectWriter Expression<OutputExpression> (HTML): obj: {:?}", obj);
        ...
        match *obj {
            ...
            _ => {
                ...
                Err(try_eval_from_err!(format!("Unsupported expression in HTML writer: {:?}", obj)))
            }
        }
    }
}
...
impl ObjectWriter<Primitive, HtmlOutput> for DefaultHtmlWriter {
    fn write_object(&mut self, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, obj: &Primitive) -> DocumentProcessingResult<()> {
        debug!("ObjectWriter Primitive<OutputExpression> (HTML): obj: {:?}", obj);
...
impl ObjectWriter<ExpressionValue<OutputExpression>, HtmlOutput> for DefaultHtmlWriter {
    fn write_object(&mut self, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, obj: &ExpressionValue<OutputExpression>) -> DocumentProcessingResult<()> {
        match *obj {
            ...
            _ => Err(try_eval_from_err!(format!("Unsupported expression value when writing: {:?}", obj)))
        }
    }
}
...
impl ObjectWriter<Block<ProcessedExpression>, HtmlOutput> for DefaultHtmlWriter {
    fn write_object(&mut self, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, obj: &Block<ProcessedExpression>) -> DocumentProcessingResult<()> {
        debug!("ObjectWriter Block<ProcessedExpression> (HTML): obj: {:?}", obj);
...
fn write_event_props<'s>(_self: &'s mut DefaultHtmlWriter, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, props: &Vec<(String, ExpressionValue<OutputExpression>)>) -> DocumentProcessingResult<()> {
...
fn write_open<'s>(_self: &'s mut DefaultHtmlWriter, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, desc: &ElementDescriptor<ProcessedExpression>, is_void: bool, comp_desc: Option<&ComponentInstanceDescriptor<OutputExpression>>, idx: Option<i32>) -> DocumentProcessingResult<()> {
    ...
    for prop in desc.props() {
        ...
        if (name == "class") {
            ...
            if let ExpressionValue::Expression(Expression::Composite(CompositeValue::ObjectValue(ref props))) = *expr {
                let mut classes = Vec::with_capacity(16);
                if let Some(box ref props) = *props {
                    for prop in props {
                        eprintln!("Writing class with object: {} (a): {:?}", prop.key(), prop.value());
                        let expr: ExpressionValue<OutputExpression> = TryEvalFrom::try_eval_from(prop.value(), ctx)?;
    ...
    if let Some(events) = desc.events() {
        ...
        for event_binding in events {
            ...
            write!(w, "on{}=\"{}(event, {{", event_binding.event_name(), event_binding.key())?;

            let actions: Vec<_> = event_binding.event().actions().map(|v| v.into_iter().collect()).unwrap_or_default();
            eprintln!("Event actions: {:?}", &actions);

            let const_props: Vec<_> = actions.into_iter()
                    .flat_map(|action| {
            ...
            let const_props: Vec<(String, ExpressionValue<OutputExpression>)> = TryEvalFrom::try_eval_from(&const_props, ctx)?;
...
fn join_key(ctx: &mut OutputContext<ProcessedExpression>, key: &str) -> DocumentProcessingResult<String> {
    let binding = CommonBindings::CurrentElementKeyPath;
    let element_key;

    if let Some(ExpressionValue::Primitive(Primitive::StringVal(ref s))) = ctx.find_value(&binding)? {
        eprintln!("[html] MapInstanceComponent: existing CurrentElementKeyPath: {}", s);
        element_key = format!("{}.{}", s, key);
    } else {
        element_key = key.to_owned();
    };

    Ok(element_key)
}

impl ObjectWriter<ComponentInstanceDescriptor<ProcessedExpression>, HtmlOutput> for DefaultHtmlWriter {
    fn write_object(&mut self, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, comp_desc: &ComponentInstanceDescriptor<ProcessedExpression>) -> DocumentProcessingResult<()> {
        debug!("ObjectWriter ComponentInstanceDescriptor<ProcessedExpression> (HTML): comp_desc: {:?}", comp_desc);

        ctx.push_child_scope();

        if let Some(component_props) = comp_desc.props() {
            for prop in component_props {
                let expr: ExpressionValue<OutputExpression> = TryEvalFrom::try_eval_from(prop.expr(), ctx)?;
                let binding = CommonBindings::NamedComponentProp(prop.name().to_owned(), Default::default());
                eprintln!("[HTML] write_comp_desc: adding binding [{:?}] with value [{:?}].", binding, expr);
                ctx.bind_value(binding, expr)?;
            }
        };

        let component_tag = comp_desc.tag();
        let component = ctx.doc().component(component_tag).map(|c| c.to_owned()).expect("component expected to exist in write_comp_desc");
...
impl ObjectWriter<ElementOp<ProcessedExpression>, HtmlOutput> for DefaultHtmlWriter {
    fn write_object(&mut self, w: &mut io::Write, ctx: &mut OutputContext<ProcessedExpression>, el: &ElementOp<ProcessedExpression>) -> DocumentProcessingResult<()> {
        debug!("ObjectWriter ElementOp<ProcessedExpression> (HTML): el: {:?}", el);
        ...
        match *el {
            ...
            ElementOp::WriteValue(ref expr, ref key) => {
                ...
                eprintln!("ObjectWriter ElementOp<ProcessedExpression> (HTML) WriteValue expr: {:?}", expr);
                self.write_object(w, ctx, &expr)
            }
            ...
            ElementOp::MapInstanceComponent(ref comp_desc, ref item_key, ref coll, _) => {
                ...
                let coll: Option<Vec<ExpressionValue<OutputExpression>>> = TryEvalFrom::try_eval_from(&coll, ctx)?;
                if let Some(coll) = coll {
                    for (idx, item) in (0i32..).zip(coll.iter()) {
                        ctx.push_child_scope_with_environment(OutputScopeEnvironment::MappedComponentInstance);
                        ...
                        ctx.bind_loop_value(binding, ExpressionValue::Primitive(Primitive::Int32Val(idx)))?;
