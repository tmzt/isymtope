...
impl TryProcessFrom<ExternNode<SourceExpression>> for ExternNode<ProcessedExpression> {
    fn try_process_from(src: &ExternNode<SourceExpression>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        let children = match src.0 { Some(box ref children) => Some(Box::new(TryProcessFrom::try_process_from(children, ctx)?)), _ => None };
...
impl TryEvalFrom<ExternNode<ProcessedExpression>> for ExternNode<OutputExpression> {
    fn try_eval_from(src: &ExternNode<ProcessedExpression>, ctx: &mut OutputContext<ProcessedExpression>) -> DocumentProcessingResult<Self> {
