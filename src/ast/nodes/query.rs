...
impl TryProcessFrom<QueryDefinition<SourceExpression>> for Query<ProcessedExpression> {
    fn try_process_from(src: &QueryDefinition<SourceExpression>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        debug!("TryProcess QueryDefinition -> Query: src: {:?}", src);

        let name = src.0.to_owned();
        let params: FormalParams<ProcessedExpression> = TryProcessFrom::try_process_from(&src.1, ctx)?;
        ...
        let components: Vec<QueryComponent<ProcessedExpression>> = TryProcessFrom::try_process_from(&src.2, ctx)?;
        ctx.pop_scope();
...
impl TryProcessFrom<ParsedQueryParamValue<SourceExpression>> for QueryParamValue<ProcessedExpression> {
    fn try_process_from(src: &ParsedQueryParamValue<SourceExpression>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        eprintln!("[Query] TryProcess ParsedQueryParamValue -> QueryParamValue: src: {:?}", src);
        ...
        Ok(match *src {
            ParsedQueryParamValue::Lens(ref l) => {
                ...
                eprintln!("[Query] TryProcess ParsedQueryParamValue -> QueryParamValue: lens: {:?}", lens);

                QueryParamValue::new(default_alias, ExpressionValue::Lens(lens, Default::default()))
            }

            ParsedQueryParamValue::Prop(ref p, _) => {
                let key = p.key().to_owned();
                eprintln!("[Query] TryProcess ParsedQueryParamValue -> QueryParamValue: key: {:?}", key);
                let value = p.value();
                eprintln!("[Query] TryProcess ParsedQueryParamValue -> QueryParamValue: value: {:?}", value);
...
impl TryProcessFrom<LensQueryCall<SourceExpression>> for QueryCall<ProcessedExpression> {
    fn try_process_from(src: &LensQueryCall<SourceExpression>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        eprintln!("[Query] TryProcess LensQueryCall -> QueryCall: src: {:?}", src);
        let params: Vec<QueryParamValue<ProcessedExpression>> = TryProcessFrom::try_process_from(&src.1, ctx)?;
        eprintln!("[Query] TryProcess LensQueryCall -> QueryCall: params: {:?}", params);
