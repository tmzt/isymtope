...
pub enum QueryComponent<T> {
    CaseWhere(Box<ExpressionValue<T>>, Box<ExpressionValue<T>>, PhantomData<T>)
}
...
impl TryProcessFrom<QueryComponent<SourceExpression>> for QueryComponent<ProcessedExpression> {
    fn try_process_from(src: &QueryComponent<SourceExpression>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        debug!("TryProcess QueryComponent -> QueryComponent: src: {:?}", src);

        Ok(match *src {
            QueryComponent::CaseWhere(box ref expr, box ref cond, _) => {
                let cond: ExpressionValue<ProcessedExpression> = TryProcessFrom::try_process_from(cond, ctx)?;

                let expr = match *expr {
                    ExpressionValue::Expression(Expression::Pipeline(ref p, _)) => {
                        let reduced: ReducedPipelineValue<ProcessedExpression> = TryProcessFrom::try_process_from(p, ctx)?;
...
fn case_where(ctx: &mut OutputContext<ProcessedExpression>, cond: &ExpressionValue<ProcessedExpression>, expr: &ExpressionValue<ProcessedExpression>) -> DocumentProcessingResult<(bool, ExpressionValue<OutputExpression>)> {
    ...
    eprintln!("[Query] TryEval QueryCall -> ExpressionValue: cond value: {:?}", cond);
...
impl TryEvalFrom<QueryCall<ProcessedExpression>> for ExpressionValue<OutputExpression> {
    fn try_eval_from(query_call: &QueryCall<ProcessedExpression>, ctx: &mut OutputContext<ProcessedExpression>) -> DocumentProcessingResult<Self> {
        eprintln!("[Query] TryEval QueryCall -> ExpressionValue: query_call: {:?}", query_call);

        let name = query_call.name();
        eprintln!("[Query] TryEval QueryCall -> ExpressionValue: name: {:?}", name);

        let params: Vec<_> = query_call.params().collect();
        eprintln!("[Query] TryEval QueryCall -> ExpressionValue: params: {:?}", params);

        let query = ctx.doc().query(name) 
            .map(|v| Ok(v.to_owned()))
        ...
        for prop in params {
            let binding = CommonBindings::NamedQueryParam(prop.key().to_owned(), Default::default());
            let value: ExpressionValue<OutputExpression> = TryEvalFrom::try_eval_from(prop.value(), ctx)?;

            eprintln!("[Query] Adding binding {:?} with value [{:?}]", binding, value);
            ctx.bind_value(binding, value)?;
        }

        let acc: DocumentProcessingResult<Option<ExpressionValue<OutputExpression>>> = Ok(None);

        let res = query.components()
            .fold_while(acc, |acc, component| {
                match *component {
                    QueryComponent::CaseWhere(box ref expr, box ref cond, _) => {
                        eprintln!("[Query] TryEval QueryCall -> ExpressionValue: case cond: {:?} expr {:?}", cond, expr);
                        let res = case_where(ctx, cond, expr);
                        eprintln!("[Query] TryEval QueryCall -> ExpressionValue: res: {:?}", res);
        ...
        if res.is_none() {
            return Err(try_eval_from_err!("Unable to evaluate query, result is None."));
        }
        let res = res.unwrap();
