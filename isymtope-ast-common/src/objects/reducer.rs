...
impl<T> Reducer<T> {
    pub fn new(
        name: String,
        actions: Option<Vec<ReducerAction<T>>>,
        default_value: Option<ExpressionValue<T>>,
        shape: Option<OuterShape>,
    ) -> Self {
    ...
    pub fn default_value(&self) -> Option<&ExpressionValue<T>> {
        self.2.as_ref()
    }

    pub fn shape(&self) -> Option<OuterShape> {
        self.3
    }
}

impl<I, O> TryProcessFrom<Reducer<I>> for Reducer<O>
where
    ...
    O: ::std::fmt::Debug,
{
    fn try_process_from(
        ...
        ctx.push_child_scope_with_environment(ProcessingScopeEnvironment::Reducer(
            src.0.to_owned(),
        ));
        ...
        let default_value: Option<ExpressionValue<O>> =
...
impl<T> ReducerAction<T> {
    ...
    pub fn expr(&self) -> Option<&ExpressionValue<T>> {
        self.2.as_ref()
    }
}
...
impl<T: Debug> MapIdents<T> for ReducerAction<T> {
    fn map_idents(self, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        let formals: HashSet<String> = (&self.1)
            .params()
        ...
        for formal in formals {
            let binding =
                CommonBindings::NamedReducerActionParam(formal.to_owned(), Default::default());
            ctx.bind_ident(formal, binding)?;
        }

        // let expr = self.2.map(|e| e.map_idents(&formals, "action"));
        // let expr = self.2.map(|e| e.map_idents(ctx));
        let expr = match self.2 {
            Some(e) => Some(e.map_idents(ctx)?),
            _ => None,
        };
...
impl<I, O> TryProcessFrom<ReducerAction<I>> for ReducerAction<O>
where
    ExpressionValue<O>: TryProcessFrom<ExpressionValue<I>>,
    I: ::std::fmt::Debug,
    O: ::std::fmt::Debug,
