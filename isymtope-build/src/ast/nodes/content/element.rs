...
pub struct ElementNode<T>(
    ...
    String,
    Option<String>,
    Option<Vec<ElementAttrValue<T>>>,
    Option<Vec<ElementBinding<T>>>,
...
impl<T: Clone> ElementNode<T> {
    pub fn new(
        ...
        bindings: Option<Vec<ElementBinding<T>>>,
    ) -> Self {
...
impl TryProcessFrom<ElementNode<SourceExpression>> for ElementNode<ProcessedExpression> {
    fn try_process_from(
        src: &ElementNode<SourceExpression>,
        ctx: &mut ProcessingContext,
    ) -> DocumentProcessingResult<Self> {
        let bindings: Option<Vec<_>> = match src.4 {
            Some(ref bindings) => Some(TryProcessFrom::try_process_from(bindings, ctx)?),
            _ => None,
        };
        ...
        let props = match src.3 {
            Some(ref props) => Some(TryProcessFrom::try_process_from(props, ctx)?),
            _ => None,
        };
        let children = match src.5 {
