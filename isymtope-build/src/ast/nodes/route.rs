...
impl<I, O> TryProcessFrom<RouteDefinition<I>> for Route<O> where ExpressionValue<O>: TryProcessFrom<ExpressionValue<I>>, I: ::std::fmt::Debug, O: ::std::fmt::Debug {
...
impl<I, O> TryProcessFrom<RouteAction<I>> for RouteActionValue<O> where ExpressionValue<O>: TryProcessFrom<ExpressionValue<I>>, I: ::std::fmt::Debug, O: ::std::fmt::Debug {
    fn try_process_from(src: &RouteAction<I>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        eprintln!("TryProcess RouteAction (ast) -> RouteActionValue: src: {:?}", src);
        ...
        Ok(match *src {
            ...
            RouteAction::Actions(ref a, _) => RouteActionValue::Actions(TryProcessFrom::try_process_from(a, ctx)?, Default::default()),
        })
    }
}
