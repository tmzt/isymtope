...
impl Default for ProcessingScopeEnvironment {
    fn default() -> Self { ProcessingScopeEnvironment::Normal }
}
...
pub trait TryProcessFrom<Input> {
    fn try_process_from(src: &Input, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> where Self: Sized;
}

impl<I, O: TryProcessFrom<I>> TryProcessFrom<Option<I>> for Option<O> {
    fn try_process_from(src: &Option<I>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
        Ok(match *src {
...
impl<I, O: TryProcessFrom<I>> TryProcessFrom<Box<I>> for O {
    fn try_process_from(src: &Box<I>, ctx: &mut ProcessingContext) -> DocumentProcessingResult<Self> {
