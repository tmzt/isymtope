...
impl<T> SourceLensValue<T> {
    pub fn default_alias(&self) -> DocumentProcessingResult<String> {
        ...
        match *self {
            ...
            | SourceLensValue::QueryLens(Some(ref s), _, _) => Ok(s.to_owned()),

            SourceLensValue::GetLens(
                None,
                box ExpressionValue::Expression(Expression::Ident(ref s, _)),
                _,
            ) => Ok(s.to_owned()),
...
impl<T> LensValue<T> {
        ...
        T: ::std::fmt::Debug,
    {
        ...
        match *self {
            ...
            | LensValue::QueryLens(Some(ref s), _, _) => Ok(s.to_owned()),

            LensValue::GetLens(
                None,
                box ExpressionValue::Binding(CommonBindings::NamedReducerKey(ref s, _), _),
                _,
            ) => Ok(s.to_owned()),
            ...
            _ => {
                ...
                Err(try_process_from_err!(
                    "Unsupported LensValue for default alias"
                ))
            }
        }
    }

    pub fn expr(&self) -> DocumentProcessingResult<ExpressionValue<T>>
    where
        T: Clone,
    {
        match *self {
            LensValue::ForLens(_, box ref expr, _) | LensValue::GetLens(_, box ref expr, _) => {
                Ok(expr.to_owned())
            }

            LensValue::QueryLens(_, ref query_call, _) => Ok(ExpressionValue::Expression(
                Expression::QueryCall(query_call.to_owned(), Default::default()),
            )),
        }
    }
}
...
impl TryProcessFrom<SourceLensValue<SourceExpression>> for LensValue<ProcessedExpression> {
    default fn try_process_from(
        src: &SourceLensValue<SourceExpression>,
        ctx: &mut ProcessingContext,
    ) -> DocumentProcessingResult<Self> {
        match *src {
            SourceLensValue::ForLens(ref s, box ref a, _) => {
                let lens = LensValue::ForLens(
                    s.to_owned(),
            ...
            SourceLensValue::GetLens(ref s, box ref a, _) => {
                if let ExpressionValue::Expression(Expression::Ident(ref ident_key, _)) = *a {
                    if ctx.is_reducer_key(ident_key)? {
                        let expr = ExpressionValue::Binding(
                            CommonBindings::NamedReducerKey(
                                ident_key.to_owned(),
                                Default::default(),
                            ),
                            Default::default(),
                        );
                        return Ok(LensValue::GetLens(
                            s.to_owned(),
                            Box::new(expr),
                            Default::default(),
                        ));
            ...
            SourceLensValue::QueryLens(ref alias, ref query_call, _) => {
                let query_call: QueryCall<ProcessedExpression> =
                    TryProcessFrom::try_process_from(query_call, ctx)?;
