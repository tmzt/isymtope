...
use trimmer::{Context as TrimmerContext, Parser as TrimmerParser, Template as TrimmerTemplate};
...
impl InternalTemplateRenderer {
    pub fn build (document_provider: Rc<DocumentProvider>, state_provider: Option<Rc<ReducerStateProvider>>) -> DocumentProcessingResult<InternalTemplateRenderer> {
        // Initialize output context
        let mut ctx: OutputContext<ProcessedExpression> = OutputContext::create(document_provider.clone(), state_provider);
        ...
        // Preload template
        let path = Path::new("./res/templates/page/page.trimmer");
        buf.truncate(0);
        ...
        // Preload scripts
        let scripts = vec![
            Path::new("./res/templates/page/isymtope-driver-incdom.js"),
            Path::new("./res/templates/page/isymtope-routing.js"),
            Path::new("./res/templates/page/isymtope-util.js"),
        ];
        ...
        // let route_keys: HashMap<String, String> = doc.routes()
        //     .map(|v| v
        //         .map(|(pattern, route)| ( (pattern.to_owned(), route.function_key().to_owned())  ))
        //         .collect())
        //     .unwrap_or_default();
        ...
        // let mut event_funcs: HashMap<String, String> = doc.event_bindings().map(|r| (r.name().to_owned(), r)).collect();
        // let event_keys: Vec<String> = event_funcs.keys().map(|s| s.to_owned()).collect();
        ...
        // if let Some(v) = doc.event_bindings() {

        let mut event_bindings = doc.event_bindings();

        for ref event_binding in event_bindings {
            ...
            let action_keys = event_action_keys.entry(key.clone())
                .or_insert_with(|| Default::default());

            let action_bodies = event_action_bodies.entry(key.clone())
                .or_insert_with(|| Default::default());
            ...
            if let Some(actions) = event.actions() {
                ...
                let event_prop_aliases: Vec<_> = actions.iter()
                        ...
                        .filter_map(|(alias, prop)| match prop { ExpressionValue::Expression(Expression::Path(ref path_value, _)) => Some((alias, prop.to_owned(), path_value.component_string())), _ => None })
                        .collect();

                for (alias, prop, raw_path) in event_prop_aliases {
                    let binding = CommonBindings::PathAlias(alias.to_owned(), Default::default());
                    let expr = ExpressionValue::Expression(Expression::RawPath(raw_path, Default::default()));
                    // let expr = TryEvalFrom::try_eval_from(&prop, &mut ctx)?;
                    ctx.bind_value(binding, expr)?;
                }

                // for (alias, prop) in event_prop_aliases {
                //     let binding = CommonBindings::PathAlias(alias.to_owned(), Default::default());
                //     let expr = TryEvalFrom::try_eval_from(&prop, &mut ctx)?;
                //     ctx.bind_value(binding, expr)?;
                // }
        ...
        let route_func_keys: HashMap<String, String> = doc.routes().map(|r| (r.pattern().to_owned(), r.function_key().to_owned().into())).collect();
        ...
        if let Some(v) = doc.reducers() {
            ...
            for (reducer_key, reducer) in v {
                let action_keys = reducer_action_keys.entry(reducer_key.to_owned())
                    .or_insert_with(|| Default::default());

                let action_bodies = reducer_bodies.entry(reducer_key.to_owned())
                    .or_insert_with(|| Default::default());

                // let bodies = reducer_bodies.entry(reducer_key.to_owned())
                //     .or_insert_with(|| Default::default());
                ...
                if let Some(v) = reducer.actions() {
                    for action in v {
                        ...
                        if let Some(expr) = action.expr() {
                            ...
                            // let body = match *expr {
                            //     ExpressionValue::Binding(CommonBindings::CurrentReducerState(_), _) => value,
                            //     ExpressionValue::Expression(Expression::ReducedPipeline(..)) => value,
                            //     ExpressionValue::Expression(Expression::Filter(..)) => value,
                            //     _ => value.to_owned()
                            // };
        ...
        let extern_reducer_keys: Vec<_> = doc.extern_reducers().map(|n| n.name().to_owned()).collect();
        ...
        for (query_name, query) in doc.queries() {
            ...
            let body = str::from_utf8(bytes.as_slice())?.to_owned();
            let params: Vec<_> = query.params().map(|v| v.map(|s| s.to_owned()).collect()).unwrap_or_default();
    ...
    pub fn render(&self) -> DocumentProcessingResult<(String)> {
        ...
        tctx.set("JS_PRELOAD_SRC", &self.preload_src);
        ...
        let out_buf = self.template.render(&tctx)?;
        Ok(out_buf)
    }
}